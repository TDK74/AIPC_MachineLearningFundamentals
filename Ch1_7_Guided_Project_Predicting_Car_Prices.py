...
def knn_train_test_hpt(train_cols, target_col, df):
    np.random.seed(1)
    ...
    for k in k_values:
        ...
        # Calculate and return RMSE.
        mse_hpt = MSE(test_df[target_col], predicted_labels)
...
for nr_best_feats in range(2, 6):
    k_rmse_results_hpt['{} best features'.format(nr_best_feats)] = knn_train_test_hpt(
        sorted_features[:nr_best_feats], 'price', numeric_cars)

# for normal run
print(f"RMSE results from Hyperparameter Tuning of Mulivariate model:\n{k_rmse_results_hpt}")
